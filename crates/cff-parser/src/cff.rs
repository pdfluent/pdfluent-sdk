    ...
    pub fn glyph_width(&self, glyph_id: GlyphId) -> Option<u16> {
        match self.kind {
            ...
            FontKind::CID(ref cid) => {
                // For CID-keyed CFF fonts, each glyph belongs to a Font DICT (FD)
                // identified by FDSelect. Each FD has its own Private DICT with
                // DefaultWidth and NominalWidth. The charstring width operand (if
                // present) is added to the NominalWidth of the glyph's own FD.
...
mod width_tests {
    ...
    /// Minimal CID-keyed CFF binary with 2 glyphs and 1 Font DICT:
    ///
    /// - GID 0: endchar without width operand → DefaultWidth = 500
    /// - GID 1: push 650, endchar → NominalWidth(0) + 650 = 650
    ///
    /// FD Private DICT: DefaultWidth=500, NominalWidth=0
    ///
    /// Layout (64 bytes total):
    ///   [0]  Header
    ///   [4]  Name INDEX ("F")
    ///   [10] Top DICT INDEX (ROS, charset=35, FDSelect=38, FDArray=51, CharStrings=41)
    ///   [31] String INDEX (empty)
    ///   [33] GlobalSubr INDEX (empty)
    ///   [35] Charset Format 0 (GID1 → CID 1)
    ///   [38] FDSelect Format 0 (GID0,GID1 → FD 0)
    ///   [41] CharStrings INDEX (2 glyphs)
    ///   [51] FDArray INDEX (1 Font DICT: Private 6 @ 59)
    ///   [59] Private DICT (DefaultWidth=500, NominalWidth=0)
    #[rustfmt::skip]
    const MINIMAL_CID_CFF: &[u8] = &[
        // [0] Header: major=1, minor=0, hdrSize=4, offSize=1
        0x01, 0x00, 0x04, 0x01,

        // [4] Name INDEX: count=1, offSize=1, offsets=[1,2], data="F"
        0x00, 0x01, 0x01, 0x01, 0x02, 0x46,

        // [10] Top DICT INDEX: count=1, offSize=1, offsets=[1,17]
        0x00, 0x01, 0x01, 0x01, 0x11,
        // Top DICT data (16 bytes):
        // ROS: Adobe(SID 66) Identity(SID 228) 0 — operator 1230 (12 30)
        0xCD, 0xF7, 0x78, 0x8B, 0x0C, 0x1E,
        // charset offset 35 — operator 15
        0xAE, 0x0F,
        // FDSelect offset 38 — operator 1237 (12 37)
        0xB1, 0x0C, 0x25,
        // FDArray offset 51 — operator 1236 (12 36)
        0xBE, 0x0C, 0x24,
        // CharStrings offset 41 — operator 17
        0xB4, 0x11,

        // [31] String INDEX: empty (count=0)
        0x00, 0x00,

        // [33] GlobalSubr INDEX: empty (count=0)
        0x00, 0x00,

        // [35] Charset Format 0: GID 1 → CID 1
        0x00, 0x00, 0x01,

        // [38] FDSelect Format 0: GID 0 → FD 0, GID 1 → FD 0
        0x00, 0x00, 0x00,

        // [41] CharStrings INDEX: count=2, offSize=1, offsets=[1,2,5]
        0x00, 0x02, 0x01, 0x01, 0x02, 0x05,
        // GID 0: endchar (no width operand → DefaultWidth)
        0x0E,
        // GID 1: push 650, endchar (NominalWidth + 650 = 650)
        0xF9, 0x1E, 0x0E,

        // [51] FDArray INDEX: count=1, offSize=1, offsets=[1,4]
        0x00, 0x01, 0x01, 0x01, 0x04,
        // Font DICT: Private size=5 @ offset=59 — operator 18
        // encode(5)=0x90, encode(59)=0xC6, op18=0x12
        0x90, 0xC6, 0x12,

        // [59] Private DICT: DefaultWidth=500 (op 20), NominalWidth=0 (op 21)
        0xF8, 0x88, 0x14,   // 500 op20
        0x8B, 0x15,         // 0   op21
    ];

    #[test]
    fn cid_glyph_width_default() {
        // GID 0 has no width operand in its charstring → should return DefaultWidth=500.
        let table = Table::parse(MINIMAL_CID_CFF).expect("CID CFF should parse");
        assert_eq!(
            table.glyph_width(GlyphId(0)),
            Some(500),
            "GID 0 must return DefaultWidth=500"
        );
    }

    #[test]
    fn cid_glyph_width_nominal_plus_delta() {
        // GID 1 charstring: push 650, endchar → NominalWidth(0) + 650 = 650.
        let table = Table::parse(MINIMAL_CID_CFF).expect("CID CFF should parse");
        assert_eq!(
            table.glyph_width(GlyphId(1)),
            Some(650),
            "GID 1 must return NominalWidth(0) + 650 = 650"
        );
    }

    #[test]
    fn cid_glyph_width_out_of_range() {
        // GID beyond the number of glyphs must return None.
        let table = Table::parse(MINIMAL_CID_CFF).expect("CID CFF should parse");
        assert_eq!(
            table.glyph_width(GlyphId(2)),
            None,
