    ...
    fn decode_filters(&self, limit: Option<usize>) -> Result<Vec<u8>> {
        ...
        for filter in filters {
            output = match filter {
                ...
                // Filters this fork decodes and upstream does not. They take no
                // `limit` of their own; the `output.len() > max` check below this
                // match bounds them, which is sufficient because none of them is
                // a compression filter with unbounded expansion -- unlike Flate
                // and LZW, they cannot allocate past the limit before returning.
...
mod test {
    ...
    fn decompress_zlib_within_limit_succeeds() {
        ...
        let result = Stream::decompress_zlib(&compressed, None, Some(MAX_DECOMPRESSED_BYTES));
