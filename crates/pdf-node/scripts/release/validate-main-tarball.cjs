...
try {
  ...
  // On Windows `npm` is a .cmd shim: execFileSync does no PATHEXT resolution
  // (bare name fails with ENOENT), and even the .cmd name needs `shell: true`
  // since Node refuses to spawn a .cmd/.bat file directly (EINVAL).
