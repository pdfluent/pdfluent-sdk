...
impl<'a> FormMerger<'a> {
    ...
    // upper limit.
    //
    // fixes #812: materialize those copies as real siblings in the parent
    // container, not inside an extra synthetic `*_container` wrapper. The
    // wrapper changed layout semantics for table rows and positioned children:
    // table layout treated repeated rows as cells of one row, which collapsed
    // content and caused under-pagination. Real sibling instances preserve the
    // original parent layout the same way Adobe/XFA does.
...
<xfa:datasets xmlns:xfa="http://www.xfa.org/schema/xfa-data/1.0/">
  <xfa:data>
    ...
    #[test]
    fn repeating_table_rows_expand_as_sibling_rows_not_wrapper_cells() {
        let template = r#"<?xml version="1.0"?>
<template xmlns="http://www.xfa.org/schema/xfa-template/3.3/">
  <subform name="form1" layout="tb">
    <pageSet>
