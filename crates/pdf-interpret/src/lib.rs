...
example, to render PDF files to bitmaps (which is what the `pdf-interpret` crate does), or other formats
such as SVG.
...
the implementation of `pdf-interpret` and `pdf-interpret-svg` to get an idea on how to use this crate.
...
- `embed-fonts`: PDF processors are required to support 14 predefined fonts that do not need to be
  embedded into a PDF file. If you enable this feature, pdf-interpret will embed a (permissively-licensed)
