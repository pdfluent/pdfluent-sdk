...
/// [`AnnotationBuilder`] (`square`, `highlight`, `link_uri`, …) instead
...
pub enum AnnotSubtype {
    ...
    /// region marking. Pair with [`AnnotationBuilder::interior_color`]
    /// for a fill.
    Square,
    /// Filled or stroked ellipse (`/Circle`) inscribed in the
    /// rectangle. Pair with [`AnnotationBuilder::interior_color`] for a
    /// fill.
    Circle,
    /// Line segment (`/Line`) between two points, optionally with arrow
    /// or other end caps. See [`AnnotationBuilder::line`] and
    /// [`AnnotationBuilder::line_endings`].
    Line,
    /// Text-markup highlight (`/Highlight`) — semi-transparent yellow
    /// fill over a text range. Use [`AnnotationBuilder::quad_points`]
    ...
    /// [`AnnotationBuilder::sticky_note`].
    Text,
    /// Rubber stamp annotation (`/Stamp`) — APPROVED, DRAFT,
    /// CONFIDENTIAL, etc. See [`StampName`] for the standard set and
    /// [`AnnotationBuilder::stamp`] / [`AnnotationBuilder::stamp_custom`].
    Stamp,
    /// Free-hand ink annotation (`/Ink`) consisting of one or more
    /// stroke paths. See [`AnnotationBuilder::ink`].
    Ink,
    /// Closed polygon (`/Polygon`) defined by a list of vertices.
    /// Optionally filled with [`AnnotationBuilder::interior_color`].
    Polygon,
    ...
    /// [`AnnotationBuilder::link_uri`] and
    /// [`AnnotationBuilder::link_dest`].
    Link,
}
...
/// Standard rubber-stamp names per ISO 32000-2 §12.5.6.12.
///
/// PDF viewers (Adobe, Foxit, etc.) ship built-in appearances for these
/// names; using a standard name produces consistent visuals across
/// viewers. For arbitrary text, use
/// [`AnnotationBuilder::stamp_custom`] instead — but be aware viewers
/// will render it with a generic appearance.
#[cfg(feature = "write")]
#[derive(Debug, Clone, Copy)]
pub enum StampName {
...
///
/// Pass two values to [`AnnotationBuilder::line_endings`] — the first
/// applies to the start point, the second to the end point. The
/// "Reversed" variants (`R*`) have the visual oriented in the opposite
/// direction of the line — useful for double-arrow lines where both
/// ends should point inward.
#[cfg(feature = "write")]
#[derive(Debug, Clone, Copy)]
pub enum LineEnding {
    ...
    Diamond,
    /// `/OpenArrow` — open V-shaped arrowhead pointing away from the
    /// line.
    OpenArrow,
    /// `/ClosedArrow` — filled triangular arrowhead pointing away from
    /// the line.
    ClosedArrow,
    /// `/Butt` — short perpendicular tick at the end of the line.
    Butt,
    /// `/ROpenArrow` — reversed open arrow (points back along the
    /// line).
    ROpenArrow,
    /// `/RClosedArrow` — reversed filled arrow (points back along the
    /// line).
    RClosedArrow,
