...
impl<'a> ArithmeticDecoder<'a> {
    ...
    #[inline(always)]
    // Upstream's thin wrapper over decode(). Our call sites use decode()
    // directly; kept so the file stays identical to upstream and the
    // next merge is a no-op rather than a conflict.
    #[allow(dead_code)]
