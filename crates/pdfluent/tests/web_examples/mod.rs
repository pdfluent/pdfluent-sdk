...
// The 5 snippets below are generated by tools/pdfluent-snippet-extract (#1236)
// and must not be edited by hand.  They are intentionally NOT compiled here
// because pdfluent.com currently shows aspirational API (PdfPermissions,
// RenderOptions, page.render(), …) not yet in the SDK.  The Docs Drift Guard
// verifies byte-for-byte that committed files match the extractor output;
// this exclusion prevents compilation failures while the API gap exists.
// Restore when the website snippets compile against the current SDK.
//
