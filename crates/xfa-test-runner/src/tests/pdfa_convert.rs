...
impl PdfTest for PdfAConvertTest {
    ...
    fn run(&self, pdf_data: &[u8], path: &Path) -> TestResult {
        ...
        // CharStrings INDEX), so it cannot corrupt charstring operators. (#465)
        set_progress("cff_invalid_bcd");
        let _ = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        ...
        // 3a3c. Final-pass TrueType width correction — only corrects widths
        // where the glyph IS present in the font, using Differences + BaseEncoding.
        // Safe: never sets width to 0 for absent glyphs. (#598)
        set_progress("remaining_tt_widths");
        let _ = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            pdf_manip::pdfa_fonts::fix_remaining_tt_width_mismatches(&mut doc)
        }));
            ...
            if let Some(lopdf::Object::Name(bf)) = dict.get(b"BaseFont").ok() {
                if bf == b"Symbol" {
                    ...
                    if let Some(lopdf::Object::Array(w)) = dict.get(b"Widths").ok() {
                        let sample: Vec<i64> = w.iter().take(5).map(|o| match o {
                            lopdf::Object::Integer(i) => *i, _ => -1,
                        }).collect();
                        eprintln!("  [dbg] post-embed Symbol {:?} enc={} w[0..5]={:?} total={}", id, has_enc, sample, w.len());
                    } else {
                        eprintln!("  [dbg] post-embed Symbol {:?} enc={} NO widths", id, has_enc);
                    }
                }
            }
        }
