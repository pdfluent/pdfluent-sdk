...
fn debug_tt_width_only() {
        ...
        Ok(pdf) => {
            ...
            for issue in &report.issues {
                if issue.rule.contains("6.2.11.5") {
                    println!("  [{}] {}: {}", issue.rule, issue.severity as u8, issue.message);
                }
            }
            println!("  §6.2.11.5 errors: {}", report.issues.iter().filter(|i| i.rule.contains("6.2.11.5")).count());
        }
    ...
    for (id, obj) in &doc.objects {
        ...
        let base = dict.get(b"BaseFont").ok().and_then(|o| {
            if let Object::Name(n) = o { Some(String::from_utf8_lossy(n).to_string()) } else { None }
        }).unwrap_or_default();
        let fc = match dict.get(b"FirstChar").ok() {
            Some(Object::Integer(i)) => *i as u32,
        ...
        let widths: Vec<i64> = match dict.get(b"Widths").ok() {
            Some(Object::Array(a)) => a.iter().map(|o| match o {
                Object::Integer(i) => *i, Object::Real(r) => *r as i64, _ => -1,
            }).collect(),
            Some(Object::Reference(r)) => doc.get_object(*r).ok()
                .and_then(|o| if let Object::Array(a) = o {
                    Some(a.iter().map(|o| match o { Object::Integer(i) => *i, Object::Real(r) => *r as i64, _ => -1 }).collect())
        ...
        if fc <= 33 {
            ...
            if idx < widths.len() {
                println!("  Font {:?} base={} fc={}: width[code33]={}", id, base, fc, widths[idx]);
            }
        }
    }
}
...
fn debug_round23_failures() {
    ...
    for (path, label) in pdfs {
        ...
        let orig_pdf = match Pdf::new(data.clone()) {
            Ok(p) => p,
            Err(e) => { println!("  Pdf::new failed on original: {e:?}"); continue; }
        };
        let detected = pdf_compliance::detect_pdfa_level(&orig_pdf);
        let target_level = detected.unwrap_or(pdf_compliance::PdfALevel::A2b);
        println!("  Detected level: {:?} → converting to {:?}", detected, target_level);

        let conformance = match target_level {
            pdf_compliance::PdfALevel::A1b | pdf_compliance::PdfALevel::A1a => PdfAConformance::A1b,
            pdf_compliance::PdfALevel::A3b | pdf_compliance::PdfALevel::A3u | pdf_compliance::PdfALevel::A3a => PdfAConformance::A3b,
            _ => PdfAConformance::A2b,
        };
        let is_part1 = matches!(target_level, pdf_compliance::PdfALevel::A1b | pdf_compliance::PdfALevel::A1a);

        // Load + convert
        let mut doc = match lopdf::Document::load_mem(&data) {
            Ok(d) => d,
            Err(e) => { println!("  lopdf load failed: {e:?}"); continue; }
        };
        ...
        match Pdf::new(saved) {
            Ok(pdf) => {
                ...
                println!("  Compliant: {} | Errors: {}", report.compliant, report.issues.len());
    ...
    for (id, obj) in &doc.objects {
        if let Object::Dictionary(dict) = obj {
            let subtype = dict.get(b"Subtype").ok()
                .and_then(|o| if let Object::Name(n) = o { Some(String::from_utf8_lossy(n).to_string()) } else { None })
                .unwrap_or_default();
            if !matches!(subtype.as_str(), "Type1" | "TrueType" | "Type1C" | "CIDFontType0C" | "CIDFontType2" | "MMType1") {
                continue;
            }
            let base = dict.get(b"BaseFont").ok()
                .and_then(|o| if let Object::Name(n) = o { Some(String::from_utf8_lossy(n).to_string()) } else { None })
            ...
            println!("  [{id:?}] {subtype} BaseFont={base:?} FC={fc} LC={lc} Enc={enc:?} HasFD={has_ff}");
            if fc <= 39 && 39 <= lc {
    ...
    for (id, obj) in &doc2.objects {
        if let Object::Dictionary(dict) = obj {
            let subtype = dict.get(b"Subtype").ok()
                .and_then(|o| if let Object::Name(n) = o { Some(String::from_utf8_lossy(n).to_string()) } else { None })
                .unwrap_or_default();
            if !matches!(subtype.as_str(), "Type1" | "TrueType") { continue; }
            let base = dict.get(b"BaseFont").ok()
                .and_then(|o| if let Object::Name(n) = o { Some(String::from_utf8_lossy(n).to_string()) } else { None })
                .unwrap_or_default();
            let fc = dict.get(b"FirstChar").ok()
            ...
            if fc <= 39 && 39 <= lc {
                ...
                if let Some(Object::Array(ws)) = widths_ref {
                    let idx = (39 - fc as usize);
                    if idx < ws.len() {
                        println!("  [{id:?}] {subtype} {base:?}: /Widths[{idx}] (code 39) = {:?}", ws[idx]);
                    }
                }
            }
        }
...
fn debug_gen319_code39_trace() {
        ...
        for (_id, obj) in &doc.objects {
            if let Object::Dictionary(dict) = obj {
                let base = dict.get(b"BaseFont").ok()
                    .and_then(|o| if let Object::Name(n) = o { Some(String::from_utf8_lossy(n).to_string()) } else { None })
                    .unwrap_or_default();
                if !base.contains("Helvetica") { continue; }
                let fc = dict.get(b"FirstChar").ok()
                    .and_then(|o| if let Object::Integer(i) = o { Some(*i) } else { None })
                    .unwrap_or(-1);
                let lc = dict.get(b"LastChar").ok()
                    .and_then(|o| if let Object::Integer(i) = o { Some(*i) } else { None })
                    .unwrap_or(-1);
                if fc > 39 || 39 > lc { continue; }
                if let Some(Object::Array(ws)) = dict.get(b"Widths").ok() {
                    let idx = (39 - fc) as usize;
                    if idx < ws.len() {
                        if let Object::Integer(w) = &ws[idx] { return Some(*w); }
                        if let Object::Real(w) = &ws[idx] { return Some(*w as i64); }
                    }
                }
            }
        }
    ...
    let _ = pdf_manip::pdfa_cleanup::cleanup_for_pdfa(&mut doc, false); step!("cleanup", doc);
    let _ = pdf_manip::pdfa_fonts::embed_fonts(&mut doc); step!("embed_fonts", doc);
    pdf_manip::pdfa_fonts::fix_cff_widths(&mut doc); step!("fix_cff_widths", doc);
    pdf_manip::pdfa_fonts::fix_truetype_cid_widths(&mut doc);
    pdf_manip::pdfa_fonts::fix_type1_charset(&mut doc);
    pdf_manip::pdfa_fonts::fix_truetype_encoding(&mut doc);
    let _ = pdf_manip::pdfa_fonts::fix_existing_symbolic_truetype_cmaps(&mut doc);
    ...
    pdf_manip::pdfa_fonts::fix_symbolic_flags(&mut doc);
    let _ = pdf_manip::pdfa_fonts::fix_classic_symbolic_base14_encoding(&mut doc);
    pdf_manip::pdfa_fonts::fix_missing_simple_font_widths(&mut doc); step!("fix_missing_simple_font_widths", doc);
    let _ = pdf_manip::pdfa_fonts::fix_type3_font_widths(&mut doc);
    pdf_manip::pdfa_fonts::fix_font_width_mismatches(&mut doc); step!("fix_font_width_mismatches", doc);
    pdf_manip::pdfa_fonts::fix_symbolic_font_widths(&mut doc); step!("fix_symbolic_font_widths", doc);
}
                    ...
                    if ff3 {
                        if let Some(Object::Reference(ff3_ref)) = fd.get(b"FontFile3").ok() {
                            ...
                            if let Some(Object::Stream(s)) = doc.objects.get(ff3_ref) {
                                let subtype2 = s.dict.get(b"Subtype").ok()
                                    .and_then(|o| if let Object::Name(n) = o { Some(String::from_utf8_lossy(n).to_string()) } else { None })
                                    .unwrap_or_default();
                                println!("  FontFile3 stream len={} subtype={subtype2:?}", s.content.len());
                            }
                        }
                    }
                }
...
fn debug_gen319_cff_width_direct() {
    ...
    let scale = matrix.sx;
    println!("num_glyphs={num_glyphs} matrix.sx={:.4} (scale from 1000/upem would be {:.4})", scale, 1000.0 * scale);
    ...
    for gid in 0..num_glyphs {
        ...
        println!("  GID {gid}: name={name:?} width={w:?} (scaled={:.1})", w.unwrap_or(0) as f64 * 1000.0 * scale as f64);
    }
...
fn debug_gen319_cff_glyph_index() {
    ...
    if let Some(cff) = cff_parser::Table::parse(&font_data) {
        ...
        if let Some(gid) = gix39 {
            ...
            println!("  width at gid={gid:?}: {w:?} scaled={:.1}", w.unwrap_or(0) as f64 * 1000.0 * scale);
        }
    } else {
...
fn debug_gen319_helvetica_flags() {
    ...
    for (_id, obj) in &doc.objects {
        if let Object::Dictionary(dict) = obj {
            ...
            if let Some(Object::Reference(fd_ref)) = dict.get(b"FontDescriptor").ok() {
                if let Some(Object::Dictionary(fd)) = doc.objects.get(fd_ref) {
                    let flags = fd.get(b"Flags").ok()
                        .and_then(|o| if let Object::Integer(i) = o { Some(*i) } else { None });
                    println!("Flags={flags:?} bit18(262144)={}", flags.map(|f| (f & 262144) != 0).unwrap_or(false));
                }
            }
        }
    }
...
fn debug_gen319_fix_trace() {
    ...
    for (id, obj) in &doc.objects {
        if let Object::Dictionary(dict) = obj {
            ...
            let widths_obj = dict.get(b"Widths").ok().map(|o| match o {
                Object::Array(arr) => format!("array[{}] code39={:?}", arr.len(), arr.get((39-fc) as usize)),
                Object::Reference(r) => format!("ref({} {})", r.0, r.1),
                _ => "other".to_string(),
            }).unwrap_or("none".to_string());
...
fn debug_gen319_direct_obj49() {
            ...
            Some(Object::Dictionary(dict)) => {
                ...
                match dict.get(b"Widths").ok() {
                    Some(Object::Array(ws)) => {
                        ws.get(idx).map(|o| format!("{o:?}")).unwrap_or("out_of_bounds".to_string())
                    }
                    Some(Object::Reference(r)) => {
                        match doc.objects.get(r) {
                            Some(Object::Array(ws)) => {
                                ws.get(idx).map(|o| format!("{o:?}")).unwrap_or("oob_ref".to_string())
                            }
                            _ => format!("ref({r:?})->?")
                        }
                    }
    ...
    let _ = pdf_manip::pdfa_fonts::fix_classic_symbolic_base14_encoding(&mut doc);
    pdf_manip::pdfa_fonts::fix_missing_simple_font_widths(&mut doc);
    println!("After fix_missing_simple_font_widths (49,0)[39]: {}", get_code39_at_49(&doc));
    let _ = pdf_manip::pdfa_fonts::fix_type3_font_widths(&mut doc);
    println!("Before fix_font_width_mismatches (49,0)[39]: {}", get_code39_at_49(&doc));
    let n = pdf_manip::pdfa_fonts::fix_font_width_mismatches(&mut doc);
    println!("After fix_font_width_mismatches (fixed={n}) (49,0)[39]: {}", get_code39_at_49(&doc));
    println!("Object (49,0) still font? {:?}",
        doc.objects.get(&(49, 0)).map(|o| matches!(o, Object::Dictionary(_))));
}
...
fn debug_gen319_cff_trace() {
    ...
    if let Some(gid) = code39_glyph_index {
        ...
        println!("  => width = {w:?}  rounded = {:?}", w.map(|x| x.round() as i64));
    }
}
...
fn debug_gen319_after_preprocess() {
        ...
        match doc.objects.get(&(49, 0)) {
            Some(Object::Dictionary(dict)) => {
                ...
                let w39_val = match dict.get(b"Widths").ok() {
                    Some(Object::Array(ws)) => ws.get((39 - fc) as usize).map(|o| format!("{o:?}")).unwrap_or("oob".to_string()),
                    other => format!("{other:?}"),
                };
    ...
    if let Some(Object::Dictionary(dict)) = doc.objects.get(&(49, 0)) {
        if let Some(Object::Dictionary(enc_dict)) = dict.get(b"Encoding").ok() {
            let base = enc_dict.get(b"BaseEncoding").ok()
                .and_then(|o| if let Object::Name(n) = o { Some(String::from_utf8_lossy(n).to_string()) } else { None });
    ...
    if let Some(ff3_id) = ff3_id_opt {
        if let Some(Object::Stream(s)) = doc.objects.get_mut(&ff3_id) {
            ...
            println!("  FontFile3 len={} first4={:?}", font_data.len(), &font_data[..4.min(font_data.len())]);
            let ttf_ok = ttf_parser::Face::parse(&font_data, 0).is_ok();
            ...
            if let Some(cff) = cff_parser::Table::parse(&font_data) {
                ...
                let gix39 = cff.glyph_index(39u8);
                let w39 = gix39.and_then(|g| cff.glyph_width(g)).map(|w| w as f64 * scale);
    ...
    for (id, obj) in &doc.objects {
        if let Object::Dictionary(dict) = obj {
            let st = dict.get(b"Subtype").ok()
                .and_then(|o| if let Object::Name(n) = o { Some(String::from_utf8_lossy(n).to_string()) } else { None })
                .unwrap_or_default();
            if !matches!(st.as_str(), "Type1" | "TrueType") { continue; }
            let base = dict.get(b"BaseFont").ok()
                .and_then(|o| if let Object::Name(n) = o { Some(String::from_utf8_lossy(n).to_string()) } else { None })
                .unwrap_or_default();
            let fc = dict.get(b"FirstChar").ok()
            ...
            let widths_info = match dict.get(b"Widths").ok() {
                ...
                Some(Object::Reference(r)) => {
                    match doc.objects.get(r) {
                        Some(Object::Array(ws)) => format!("ref[{}] code39={:?}", ws.len(), ws.get(idx)),
                        _ => format!("ref({r:?})->?"),
                    }
                }
