...
fn check_codes(doc: &lopdf::Document, font_name_needle: &str, codes: &[u32]) {
    for (id, obj) in &doc.objects {
        ...
        let base = dict
            ...
            .unwrap_or_default();
        if !base.contains(font_name_needle) {
            continue;
        }
        let Ok(lopdf::Object::Name(sub)) = dict.get(b"Subtype") else {
            continue;
        };
        if !matches!(sub.as_slice(), b"TrueType" | b"Type1" | b"MMType1") {
            continue;
        }
        ...
        let widths = match dict.get(b"Widths").ok() {
            ...
            Some(lopdf::Object::Reference(r)) => doc.get_object(*r).ok().and_then(|o| {
        ...
        let enc = match dict.get(b"Encoding").ok() {
            ...
            Some(lopdf::Object::Dictionary(d)) => format!(
                "Dict(base={})",
                d.get(b"BaseEncoding")
                    .ok()
                    .and_then(|o| if let lopdf::Object::Name(n) = o {
                        Some(String::from_utf8_lossy(n).to_string())
                    } else {
                        None
                    })
