...
fn set_font_width(doc: &mut Document, font_name: &str, code: u32, new_w: i64) -> bool {
    ...
    for id in ids {
        let (fc, widths_ref_opt) = {
            let Some(Object::Dictionary(dict)) = doc.objects.get(&id) else { continue };
            let base = dict.get(b"BaseFont").ok()
                .and_then(|o| if let Object::Name(n) = o { Some(String::from_utf8_lossy(n).to_string()) } else { None })
                .unwrap_or_default();
            if !base.contains(font_name) { continue }
            let fc = dict.get(b"FirstChar").ok()
                .and_then(|o| if let Object::Integer(i) = o { Some(*i as u32) } else { None }).unwrap_or(0);
...
fn verapdf_check_full(path: &str) -> (bool, Vec<String>) {
    let out = std::process::Command::new("verapdf")
        ...
        .output().unwrap_or_else(|_| panic!("verapdf not found"));
    ...
    while let Some(i) = s[pos..].find("\"failedChecks\"") {
        ...
        if let Some(colon) = region.find(':') {
            ...
            let num_end = region[num_start..].find(|c: char| !c.is_ascii_digit() && c != ' ')
                .map(|e| num_start + e).unwrap_or(num_start + 5);
            let num_str = region[num_start..num_end].trim();
            if let Ok(n) = num_str.parse::<u64>() {
                if n > 0 {
                    // Find clause nearby
                    let clause_pos = s[..region_start].rfind("\"clause\"").unwrap_or(region_start);
                    let clause_region = &s[clause_pos..region_start+20];
                    contexts.push(format!("failed={n} clause=...{}", &clause_region[..clause_region.len().min(80)]));
                }
            }
        }
        pos = region_start + 1;
...
fn main() {
    ...
    for &w in &[0i64, 250, 278, 333, 500, 722, 778] {
        ...
        if !changed { println!("  w={w}: NOT CHANGED"); continue; }
        ...
        for ctx_match in json.match_indices("\"context\":") {
            ...
            if let Some(end) = ctx_rest.find('"') {
                ...
                if !ctx.is_empty() && ctx.contains("false") {
                    let short = if ctx.len() > 100 { &ctx[ctx.len()-100..] } else { ctx };
                    ctxs.push(short.to_string());
                }
            }
        }
        println!("  w={w}: {} failures={fail_count} ctxs={:?}", 
            if compliant { "PASS✓" } else { "fail " }, &ctxs[..ctxs.len().min(3)]);
