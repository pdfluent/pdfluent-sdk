...
# Alleen lokaal, met opzet: een CI-runner checkt een verse kloon uit en heeft
# geen worktrees, dus daar beschermt dit niets. De fout die het vangt hoort bij
# deze machine, waar meerdere sessies dezelfde .git/config delen.
run gitconf  python3 scripts/ci/de_gedeelde_config_breekt_geen_worktrees.py
...
run forkpunt  python3 scripts/ci/een_fork_zonder_forkpunt_wordt_niet_gemerged.py
